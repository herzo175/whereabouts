...
**Goal:** Replace newly generated single-target cases with a five-round daily similarity challenge while preserving the ability to load historical schema-version-1 cases.

**Architecture:** Introduce a schema-version-2 case and progress contract alongside the legacy types. New generation emits five distinct rounds over one 25-location board; the web route selects the matching legacy or five-round game screen. Similarity tiers and explanations remain precomputed in source-controlled case artifacts, while scoring and progress are deterministic browser-side functions.
...
- [ ] **Step 3: Add `LegacyDailyCase`, `FiveRoundDailyCase`, `DailyRound`, and `RoundResult` types.** Define `DailyCase` as their union and branch `dailyCaseSchema.parse` on `schemaVersion`. Preserve the existing version-1 parser unchanged and add a version-2 parser enforcing exact board and relationship coverage.
...
- [ ] **Step 3: Implement progress schema version 2** as `{ schemaVersion: 2, caseDate, caseRevision, guesses: [{ roundId, poiId, tier, points }], completedAt? }` and the deterministic engine functions. Keep legacy version-1 exports operational.
- [ ] **Step 4: Extend browser persistence** to parse either progress version, reset data whose case revision or schema is incompatible, and retain the same date-scoped storage key.
...
- [ ] **Step 5: Extend publication validation** to validate either legacy cases or version-2 cases, including five targets, all 25 results per round, source resolution, images, and tier distributions.
- [ ] **Step 6: Run content-tools tests and validation** and expect all to pass against both historical cases and a version-2 fixture.
...
- [ ] **Step 3: Implement `FiveRoundGameScreen`** using the version-2 engine and persisted progress. Keep the existing `GameScreen` as a schema-version router so historical version-1 routes still render the legacy game.
...
- [ ] **Step 2: Run the focused share tests** and confirm the legacy attempt-based formatter fails.
- [ ] **Step 3: Add the version-2 share formatter and score panel** while preserving legacy sharing for historical cases.
...
- [ ] **Step 2: Replace the legacy six-attempt journeys** with desktop and mobile five-round journeys covering correct and incorrect guesses, reveal details, restoration, final score, and clipboard output.
...
### Task 7: Generate the first five-round revision

**Files:**
- Create: `packages/case-content/content/cases/2026-08-14/v7.json`
- Modify: `packages/case-content/content/manifest.json`

- [ ] **Step 1: Confirm revision 7 is unused** by checking that `packages/case-content/content/cases/2026-08-14/v7.json` does not exist.
- [ ] **Step 2: Run generation** with `pnpm content:generate-range -- --from 2026-08-14 --days 1 --revision 7` using the configured OpenRouter environment.
- [ ] **Step 3: Format and validate the generated artifact**, update the manifest to revision 7, and run `pnpm content:validate`.
- [ ] **Step 4: Run `pnpm quality` and the focused E2E journey** against the new manifest-selected case.
- [ ] **Step 5: Commit** with `git commit -m "content: publish five-round Whereabouts case 2026-08-14 v7"`.
