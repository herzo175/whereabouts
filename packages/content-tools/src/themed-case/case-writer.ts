...
export async function writeCaseDraft({
  ...
  const prompt = `Write a five-round themed geography case for ${theme.title}. Board records and evidence are supplied below; use only their IDs. Explicit target order: ${board.targetPoiIds.join(', ')}. Return exactly five compact rounds, in that order. Each clue must uniquely resolve to its target among the ${DAILY_BOARD_SIZE} board candidates using at least two concrete facts from that target's evidence; do not merely restate the shared theme. Lead with a broadly recognizable historical story, turning point, public role, or cultural association that a general-interest player can reason from. Use architecture, setting, purpose, and geography as supporting discriminators. Do not make obscure proper names, exact dates, measurements, capacities, counts, or technical specifications necessary to solve the clue; omit them unless they add memorable context. The target must remain resolvable if those details are removed or generalized. Keep the clue challenging, but it must not directly reveal the location through its name, an alternate name, city, country, demonym, or an unmistakable branded identifier. Each round must contain its targetPoiId, a clue text, and exactly ${DAILY_BOARD_SIZE} {poiId, similarityScore} entries covering every board ID exactly once. Set the target to 100 and every other candidate to a custom integer from 0 through 99 based on how strongly that specific place fits the clue. Use the full useful range: hot is 75–99, warm is 40–74, and cold is 0–39; every round must include all three bands and at least eight distinct incorrect scores. Do not return per-result prose or evidence arrays; the pipeline assembles those deterministically from the board.\n${board.candidates.map((candidate) => `${candidate.id}: ${candidate.name}; ${candidate.themeClaim}; source extract: ${candidate.source.extract}`).join('\n')}`;
...
export async function repairCaseDraft({
  ...
  const prompt = `Repair only these defective rounds: ${repairs.map((repair) => `${repair.roundId} (array index ${Number(/^round-([1-5])$/.exec(repair.roundId)?.[1] ?? 0) - 1}) ${repair.kind} ${repair.poiId ?? ''}: ${repair.reason}`).join('; ')}. Preserve all unaffected rounds exactly. Each repaired clue must uniquely resolve to its declared target among the ${DAILY_BOARD_SIZE} board candidates using at least two concrete facts from that target's evidence; do not merely restate the shared theme. Lead with a broadly recognizable historical story, turning point, public role, or cultural association that a general-interest player can reason from. Use architecture, setting, purpose, and geography as supporting discriminators. Do not make obscure proper names, exact dates, measurements, capacities, counts, or technical specifications necessary to solve the clue; omit them unless they add memorable context. The target must remain resolvable if those details are removed or generalized. Keep the clue challenging, but it must not directly reveal the location through its name, an alternate name, city, country, demonym, or an unmistakable branded identifier. Return replacement rounds in the same requested round order, each with a targetPoiId, clue text, and exactly ${DAILY_BOARD_SIZE} {poiId, similarityScore} entries. Set the target to 100 and every other candidate to a custom integer from 0 through 99. Include hot (75–99), warm (40–74), and cold (0–39) candidates with at least eight distinct incorrect scores per round. Do not return per-result prose or evidence arrays. Theme: ${theme.title}. Board evidence: ${board.candidates.map((candidate) => `${candidate.id}: ${candidate.name}; ${candidate.themeClaim}; ${candidate.source.extract}`).join('\n')}. Board candidate IDs (use each exactly once per replacement round): ${board.candidates.map((candidate) => candidate.id).join(', ')}. Board targets: ${board.targetPoiIds.join(', ')}. Current defective rounds: ${roundIndexes.map((index) => JSON.stringify(draft.rounds[index])).join('\n')}`;
