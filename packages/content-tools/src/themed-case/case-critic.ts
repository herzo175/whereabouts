...
Assess every candidate independently against every inclusion criterion and exclusion; tangential association is a failure. Resolve each clue independently from its text and evidence BEFORE comparing the answer to targetPoiId. A related place that is not the target fails. A clue passes only when a general-interest player can distinguish the target from the board through a recognizable historical narrative, turning point, public role, or cultural association, with physical or geographic facts used as support; obscure proper names, exact figures, or technical specifications cannot carry the identification. Set resolvableWithoutExactNumbers=true only when the same target remains resolvable after exact years, counts, and measurements are removed or generalized. The clue must not be a dead giveaway: reject a target name, alternate name, city, country, demonym, or unmistakable branded identifier. Never infer pass from missing, duplicate, malformed, or unsupported verdicts.
